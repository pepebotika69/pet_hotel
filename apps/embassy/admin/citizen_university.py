...
def restore_citizens_universities(modeladmin, request, queryset):
    ...
    modeladmin.message_user(request, f"{updated} relations restored successfully.", messages.SUCCESS)
...
def hard_delete_citizens_universities(modeladmin, request, queryset):
    ...
    if count > 0:
        ...
        modeladmin.message_user(request, f"{count} relations permanently deleted.", messages.SUCCESS)
    else:
        modeladmin.message_user(request, "No relations selected for deletion.", messages.WARNING)
...
class CitizenUniversityAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (_("Relations"), {"fields": ("citizen", "university")}),
        (_("Status"), {"fields": ("enrollment_date", "graduation_date", "stop_date", "is_active", "is_deleted")}),
    )
    ...
    def delete_model(self, request, obj):
        ...
        self.message_user(request, f"{obj} was deleted successfully (soft delete).", messages.SUCCESS)
    ...
    def delete_queryset(self, request, queryset):
        ...
        self.message_user(
            request, f"{len(ids_to_delete)} relations were deleted successfully (soft delete).", messages.SUCCESS
        )
